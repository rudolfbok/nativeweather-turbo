...
export const AirQualityModal = ({
  ...
  const [showAirQualityMetric, setShowAirQualityMetric] = useState<
    string | null
  >(null);
  ...
  const airQualityItemsList = [
    ...
    },
    {
      ...
      colorIndicator: NO2IndicatorColor(
        airQualityData!.current.nitrogen_dioxide
      ),
      units: "μg/m³",
    },
  ];
    ...
    <StyledModal
      ...
      header={
        showAirQualityMetric === null
          ...
          : (airQualityItemsList.find(
              (item) => item.name === showAirQualityMetric
            )?.description ?? "")
      }
      onClose={onClose}
    >
      {showAirQualityMetric === null && (
        <View>
          <View
            className={clsx(
              "ios:flex-row ios:flex-wrap mt-2 grid grid-cols-2 gap-4 sm:grid-cols-3"
            )}
          >
            {airQualityItemsList.map(
              ({ name, description, data, colorIndicator, units }, index) => (
                <AirQualityItem
                  key={index}
          ...
          <View
            className={clsx(
              "mt-4 flex w-full flex-row items-center justify-center"
            )}
          >
            <StyledText type="bodysecondary">
              {t("airquality.provider")}
            </StyledText>
            <Pressable
