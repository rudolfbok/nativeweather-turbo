...
export const TemperatureInfo = () => {
  ...
  const locations = stops.map(
    (_, index) => index / (stops.length - 1)
  ) as unknown as readonly [number, number, ...number[]];
  const colors = stops.map((stop) => stop.color) as unknown as readonly [
    string,
    string,
    ...string[],
  ];
  ...
  return (
    <View>
      ...
      <View
        className={clsx(
          "relative mt-2 h-14 w-full overflow-hidden rounded-2xl"
        )}
      >
        {/* <LinearGradient
					colors={colors}
        ...
        <View
          className={clsx(
            "ml-1 mr-6 flex-1 flex-row items-center justify-between md:mr-7"
          )}
        >
