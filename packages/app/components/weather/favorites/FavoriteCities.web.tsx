...
export const FavoriteCities = () => {
  ...
  const { favoritesData, handleRemoveCity, refreshFavoriteCities, loading } =
    useFavoriteCities();
  ...
  return (
    <View>
      <View
        className={clsx(
          "mb-3 flex min-h-6 flex-row items-center justify-between"
        )}
      >
        <StyledText type="screentitle">{t("favorites.title")}</StyledText>
        {favoritesData.length > 0 && (
          <StyledPressable
            onPress={() => setShowDeleteCity((prev) => !prev)}
            className={clsx("rounded-3xl p-1.5")}
          >
      ...
      {favoritesData.length === 0 && !loading && (
        <RoundView
          className={clsx("w-full items-center place-self-center p-4 md:w-fit")}
        >
