...
export const useSearchController = () => {
	...
	const handleCityPress = async (city: City) => {
		const { weatherData, error } = await fetchWeather(`${city.name}, ${city.region}, ${city.country}`);

		if (error || !weatherData) {
			console.log('Failed to fetch weather data for saved city');
			showToast({ message: 'Error loading weather for the selected city. Please try again.', type: 'error' });
			return;
		}

		// title: 'Error loading weather for the selected city. Please try again.',
	...
	const handleInputSearch = async (query: string) => {
		const { weatherData, error } = await fetchWeather(query);
	...
	const addToRecentSearches = (weatherData: WeatherData) => {
